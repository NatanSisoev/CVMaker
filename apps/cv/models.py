...
class CV(models.Model):
    ...
    alias = models.CharField(
        ...
        null=False,
        blank=False,
        help_text="Alias for the curriculum vitae",
        default="CV",
    )
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False, unique=True)

    info = models.ForeignKey(
        "CVInfo", on_delete=models.SET_NULL, null=True, blank=True, related_name="cv"
    )
    design = models.ForeignKey(
        "CVDesign", on_delete=models.SET_NULL, null=True, blank=True, related_name="cv"
    )
    locale = models.ForeignKey(
        "CVLocale", on_delete=models.SET_NULL, null=True, blank=True, related_name="cv"
    )
    settings = models.ForeignKey(
        "CVSettings", on_delete=models.SET_NULL, null=True, blank=True, related_name="cv"
    )

    sections = models.ManyToManyField(
        "sections.Section", through="sections.CVSection", related_name="cvs", blank=True
    )
...
class CVDesignManager(models.Manager):
    def create_from_data(self, user, file, alias: str):
        # Keep existing method for backward compatibility
        return self.create(user=user, alias=alias, theme="custom", design_file=file)
...
class CVInfo(models.Model):
    ...
    user = models.ForeignKey(
        ...
        related_name="cv_info",
    )
    alias = models.CharField(
        max_length=20, help_text="Alias for the information", default="default"
    )
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False, unique=True)
    ...
    location = models.CharField(
        max_length=255, null=True, blank=True, help_text="The location of the individual"
    )
    email = models.EmailField(
        null=True, blank=True, help_text="The email address of the individual"
    )
    photo = models.ImageField(
        upload_to="photos/", null=True, blank=True, help_text="Path to the individual's photo"
    )
    phone = models.CharField(
        max_length=20, null=True, blank=True, help_text="The phone number, including country code"
    )
    ...
    def serialize(self) -> dict:
        if hasattr(self, "data_file") and self.data_file and Path(self.data_file.path).exists():
            return _read_yaml_file(self.data_file.path)
...
class CVDesign(models.Model):
    ...
    theme = models.CharField(
        max_length=100, null=False, blank=False, choices=THEME_CHOICES, help_text="The theme name"
    )
    ...
    def serialize(self) -> dict:
        if self.design_file and Path(self.design_file.path).exists():
            return _read_yaml_file(self.design_file.path)
...
class CVLocale(models.Model):
    ...
    abbreviations_for_months = models.JSONField(
        blank=True,
        default=months_abbreviations_defaults,
        help_text="Abbreviations of the months in the locale.",
    )
    full_names_of_months = models.JSONField(
        blank=True,
        default=months_full_names_defaults,
    ...
    def serialize(self) -> dict:
        if self.locale_file and Path(self.locale_file.path).exists():
            return _read_yaml_file(self.locale_file.path)
...
class CVSettings(models.Model):
    ...
    bold_keywords = models.JSONField(
        default=list, blank=True, help_text="A list of keywords that will be bolded in the output"
    )
    ...
    def serialize(self) -> dict:
        ...
        if self.settings_file and Path(self.settings_file.path).exists():
            return _read_yaml_file(self.settings_file.path)
...
# --- end rendercv 2.x models-yaml shim ---
